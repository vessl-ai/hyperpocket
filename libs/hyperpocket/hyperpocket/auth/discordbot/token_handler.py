...
from hyperpocket.auth.discordbot.token_schema import DiscordbotTokenResponse, DiscordbotTokenRequest
...
class DiscordbotTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Discordbot token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: DiscordbotTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: DiscordbotTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: DiscordbotTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Discordbot token doesn't support refresh")

    def _make_auth_url(self, auth_req: DiscordbotTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> DiscordbotTokenRequest:
