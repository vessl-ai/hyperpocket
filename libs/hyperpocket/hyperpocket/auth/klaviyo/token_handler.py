...
from hyperpocket.auth.klaviyo.token_schema import KlaviyoTokenResponse, KlaviyoTokenRequest
...
class KlaviyoTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Klaviyo token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: KlaviyoTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: KlaviyoTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: KlaviyoTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Klaviyo token doesn't support refresh")

    def _make_auth_url(self, auth_req: KlaviyoTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> KlaviyoTokenRequest:
