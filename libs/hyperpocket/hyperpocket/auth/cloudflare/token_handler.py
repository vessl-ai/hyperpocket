...
from hyperpocket.auth.cloudflare.token_schema import CloudflareTokenResponse, CloudflareTokenRequest
...
class CloudflareTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Cloudflare token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: CloudflareTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: CloudflareTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: CloudflareTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Cloudflare token doesn't support refresh")

    def _make_auth_url(self, auth_req: CloudflareTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> CloudflareTokenRequest:
