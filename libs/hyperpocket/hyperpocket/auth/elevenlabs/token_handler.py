...
from hyperpocket.auth.elevenlabs.token_schema import ElevenlabsTokenResponse, ElevenlabsTokenRequest
...
class ElevenlabsTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Elevenlabs token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: ElevenlabsTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: ElevenlabsTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: ElevenlabsTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Elevenlabs token doesn't support refresh")

    def _make_auth_url(self, auth_req: ElevenlabsTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> ElevenlabsTokenRequest:
