...
from hyperpocket.auth.clickup.token_schema import ClickupTokenResponse, ClickupTokenRequest
...
class ClickupTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Clickup token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: ClickupTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: ClickupTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: ClickupTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Clickup token doesn't support refresh")

    def _make_auth_url(self, auth_req: ClickupTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> ClickupTokenRequest:
