...
from hyperpocket.auth.workiom.token_schema import WorkiomTokenResponse, WorkiomTokenRequest
...
class WorkiomTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Workiom token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: WorkiomTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: WorkiomTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: WorkiomTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Workiom token doesn't support refresh")

    def _make_auth_url(self, auth_req: WorkiomTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> WorkiomTokenRequest:
