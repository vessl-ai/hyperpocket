...
class NeonTokenAuthHandler(AuthHandlerInterface):
    name: str = "neon-token"
    description: str = "This handler is used to authenticate users using the Neon token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: NeonTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: NeonTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: NeonTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> NeonTokenRequest:
