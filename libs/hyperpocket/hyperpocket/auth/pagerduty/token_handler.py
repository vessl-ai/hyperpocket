...
from hyperpocket.auth.pagerduty.token_schema import PagerdutyTokenResponse, PagerdutyTokenRequest
...
class PagerdutyTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Pagerduty token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: PagerdutyTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: PagerdutyTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: PagerdutyTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Pagerduty token doesn't support refresh")

    def _make_auth_url(self, auth_req: PagerdutyTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> PagerdutyTokenRequest:
