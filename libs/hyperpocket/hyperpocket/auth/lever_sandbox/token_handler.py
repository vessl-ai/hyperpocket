...
from hyperpocket.auth.lever_sandbox.token_schema import (
    LeverSandboxTokenResponse,
    LeverSandboxTokenRequest,
)
...
class LeverSandboxTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = (
        "This handler is used to authenticate users using the LeverSandbox token."
    )
    scoped: bool = False
    ...
    def prepare(
        self,
        auth_req: LeverSandboxTokenRequest,
        thread_id: str,
        profile: str,
        future_uid: str,
        *args,
    ...
    async def authenticate(
        self, auth_req: LeverSandboxTokenRequest, future_uid: str, *args, **kwargs
    ) -> AuthContext:
        future_data = FutureStore.get_future(future_uid)
        access_token = await future_data.future

        response = LeverSandboxTokenResponse(access_token=access_token)
        context = LeverSandboxTokenAuthContext.from_lever_sandbox_token_response(
            response
        )
    ...
    async def refresh(
        self, auth_req: LeverSandboxTokenRequest, context: AuthContext, *args, **kwargs
    ) -> AuthContext:
        raise Exception("LeverSandbox token doesn't support refresh")

    def _make_auth_url(
        self, auth_req: LeverSandboxTokenRequest, redirect_uri: str, state: str
    ):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
