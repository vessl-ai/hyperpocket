...
from hyperpocket.auth.pandadoc.token_schema import PandadocTokenResponse, PandadocTokenRequest
...
class PandadocTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Pandadoc token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: PandadocTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: PandadocTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: PandadocTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Pandadoc token doesn't support refresh")

    def _make_auth_url(self, auth_req: PandadocTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> PandadocTokenRequest:
