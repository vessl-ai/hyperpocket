...
from hyperpocket.auth.timekit.token_schema import TimekitTokenResponse, TimekitTokenRequest
...
class TimekitTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Timekit token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: TimekitTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: TimekitTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: TimekitTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Timekit token doesn't support refresh")

    def _make_auth_url(self, auth_req: TimekitTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> TimekitTokenRequest:
