...
from hyperpocket.auth.alchemy.token_schema import AlchemyTokenResponse, AlchemyTokenRequest
...
class AlchemyTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Alchemy token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: AlchemyTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: AlchemyTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: AlchemyTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Alchemy token doesn't support refresh")

    def _make_auth_url(self, auth_req: AlchemyTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> AlchemyTokenRequest:
