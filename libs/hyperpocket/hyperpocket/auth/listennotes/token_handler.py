...
from hyperpocket.auth.listennotes.token_schema import ListennotesTokenResponse, ListennotesTokenRequest
...
class ListennotesTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Listennotes token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: ListennotesTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: ListennotesTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: ListennotesTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Listennotes token doesn't support refresh")

    def _make_auth_url(self, auth_req: ListennotesTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> ListennotesTokenRequest:
