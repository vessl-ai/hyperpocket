...
from hyperpocket.auth.hubspot.oauth2_schema import HubspotOAuth2Response, HubspotOAuth2Request
...
class HubspotOAuth2AuthHandler(AuthHandlerInterface):
    ...
    def prepare(self, auth_req: HubspotOAuth2Request, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: HubspotOAuth2Request, future_uid: str, *args, **kwargs) -> AuthContext:
        ...
        resp_json = resp.json()
        resp_typed = HubspotOAuth2Response(**(resp_json | {"redirect_uri": redirect_uri}))
        return HubspotOAuth2AuthContext.from_hubspot_oauth2_response(resp_typed)

    async def refresh(self, auth_req: HubspotOAuth2Request, context: AuthContext, *args, **kwargs) -> AuthContext:
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> HubspotOAuth2Request:
