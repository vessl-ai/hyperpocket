...
from hyperpocket.auth.hubspot.token_schema import HubspotTokenResponse, HubspotTokenRequest
...
class HubspotTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Hubspot token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: HubspotTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: HubspotTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: HubspotTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Hubspot token doesn't support refresh")

    def _make_auth_url(self, auth_req: HubspotTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> HubspotTokenRequest:
