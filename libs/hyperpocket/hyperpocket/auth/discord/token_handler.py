...
from hyperpocket.auth.discord.token_schema import DiscordTokenResponse, DiscordTokenRequest
...
class DiscordTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Discord token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: DiscordTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: DiscordTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: DiscordTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Discord token doesn't support refresh")

    def _make_auth_url(self, auth_req: DiscordTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> DiscordTokenRequest:
