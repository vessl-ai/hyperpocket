...
from hyperpocket.auth.activeloop.token_schema import ActiveloopTokenResponse, ActiveloopTokenRequest
...
class ActiveloopTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Activeloop token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: ActiveloopTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: ActiveloopTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: ActiveloopTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Activeloop token doesn't support refresh")

    def _make_auth_url(self, auth_req: ActiveloopTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> ActiveloopTokenRequest:
