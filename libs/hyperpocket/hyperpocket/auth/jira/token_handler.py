...
class JiraTokenAuthHandler(AuthHandlerInterface):
    name: str = "jira-token"
    description: str = "This handler is used to authenticate users using the Jira token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: JiraTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: JiraTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: JiraTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> JiraTokenRequest:
