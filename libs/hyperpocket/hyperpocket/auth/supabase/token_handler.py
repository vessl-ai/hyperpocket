...
from hyperpocket.auth.supabase.token_schema import SupabaseTokenResponse, SupabaseTokenRequest
...
class SupabaseTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Supabase token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: SupabaseTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: SupabaseTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: SupabaseTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Supabase token doesn't support refresh")

    def _make_auth_url(self, auth_req: SupabaseTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> SupabaseTokenRequest:
