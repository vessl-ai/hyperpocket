...
from hyperpocket.auth.spotify.oauth2_schema import SpotifyOAuth2Response, SpotifyOAuth2Request
...
class SpotifyOAuth2AuthHandler(AuthHandlerInterface):
    ...
    def prepare(self, auth_req: SpotifyOAuth2Request, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: SpotifyOAuth2Request, future_uid: str, *args, **kwargs) -> AuthContext:
        future_data = FutureStore.get_future(future_uid)
        auth_code = await future_data.future

        auth_string = f"{config().auth.spotify.client_id}:{config().auth.spotify.client_secret}"
    ...
    async def refresh(self, auth_req: SpotifyOAuth2Request, context: AuthContext, *args, **kwargs) -> AuthContext:
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> SpotifyOAuth2Request:
