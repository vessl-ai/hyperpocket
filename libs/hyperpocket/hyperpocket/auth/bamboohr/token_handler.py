...
from hyperpocket.auth.bamboohr.token_schema import BamboohrTokenResponse, BamboohrTokenRequest
...
class BamboohrTokenAuthHandler(AuthHandlerInterface):
    ...
    description: str = "This handler is used to authenticate users using the Bamboohr token."
    scoped: bool = False
    ...
    def prepare(self, auth_req: BamboohrTokenRequest, thread_id: str, profile: str,
    ...
    async def authenticate(self, auth_req: BamboohrTokenRequest, future_uid: str, *args, **kwargs) -> AuthContext:
    ...
    async def refresh(self, auth_req: BamboohrTokenRequest, context: AuthContext, *args, **kwargs) -> AuthContext:
        raise Exception("Bamboohr token doesn't support refresh")

    def _make_auth_url(self, auth_req: BamboohrTokenRequest, redirect_uri: str, state: str):
        params = {
            "redirect_uri": redirect_uri,
            "state": state,
        }
    ...
    def make_request(self, auth_scopes: Optional[list[str]] = None, **kwargs) -> BamboohrTokenRequest:
