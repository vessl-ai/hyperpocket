...
def process_tool_file(target_path: Path, language: str):
    ...
    if len(decorated_functions) > 1:
        raise ValueError("Multiple function_tool decorated functions found in target file")
...
def process_tool_directory(target_path: Path, language: str):
    ...
    if not decorated_function_file:
        raise ValueError("No function_tool decorated functions found in target directory")
...
def clean_tool_source_code(source_code: str) -> str:
    ...
    source_code = re.sub(r"^from hyperpocket\.tool .*?\n", "", source_code, flags=re.MULTILINE)
    source_code = re.sub(r"@function_tool\([\s\S]*?\)\n", "", source_code, flags=re.MULTILINE)
    source_code = re.sub(r"\nif __name__ == [\"']__main__[\"']:[\s\S]*", "", source_code, flags=re.MULTILINE)
...
def transform_function_to_pydantic(tool_source_code: str, func: Callable):
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef) and node.name == function_name:
            params = {arg.arg: arg.annotation.id for arg in node.args.args if isinstance(arg.annotation, ast.Name)}
